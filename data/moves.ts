...
export const Moves: {[moveid: string]: MoveData} = {
	...
	dig: {
		...
		condition: {
			...
			onInvulnerability(target, source, move) {
				if (['earthquake', 'magnitude', 'terraforce'].includes(move.id)) {
					return;
				}
				return false;
			},
			onSourceModifyDamage(damage, source, target, move) {
				if (move.id === 'earthquake' || move.id === 'magnitude' || move.id === 'terraforce') {
					return this.chainModify(2);
				}
			},
		},
	...
	tartantrum: {
		...
		flags: {protect: 1, mirror: 1, punch: 1, defrost: 1},
		target: "normal",
		type: "Rock",
	},
	bonesweep: {
